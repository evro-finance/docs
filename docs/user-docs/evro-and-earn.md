...
- EVRO will have native incentives via Protocol Liquidity Incentives (PLI) directed by the Moloch DAO governance, ensuring that there will always be sufficient liquidity to handle transactions.
- EVRO is Gnosis Chain native, and is built specifically for the fast and cheap Gnosis Chain ecosystem and to activate value flow in Gnosis-related tech and dApps.
...
- **Protocol Liquidity Incentives (PLI)**: Coming soon - supply liquidity for EVRO on incentivized DEXes directed by governance.
- **Stake RETVRN**: Coming soon - participate in governance and earn a share of protocol revenue.
...
Check historic rates on Liquity V2 [here](https://dune.com/liquity/liquity-v2#interest-rates). EVRO-specific analytics dashboard coming soon.
...
- Establish separate borrow markets for different collateral assets with their own market driven interest rates, using the Stability Pool backing to dynamically split redemptions across the available collaterals (see [Redemptions](/docs/user-docs/redemptions-and-delegation)).
...
In V2, the concept of Stability Pools is expanded to accommodate multiple collateral types including Liquid Staking Tokens (LSTs), stablecoins, and blue-chip assets. Each collateral asset has its own Stability Pool to distribute yield to EVRO depositors, keeping the interest revenue and liquidation proceeds inside the respective borrow market.
...
It is important to note that all EVRO holders (including Stability Pool depositors) remain dependent on EVRO maintaining its peg and are exposed to the overall protocol risk across all collateral types.
